...
extension Data {
	...
	func crc32(checksum: CRC32) -> CRC32 {
		...
		let bufferSize = count / MemoryLayout<UInt8>.size
	...
	func write(to file: UnsafeMutablePointer<FILE>) throws -> Int {
		var sizeWritten = 0
		withUnsafeBytes{ (bytes: UnsafeRawBufferPointer) in
			guard let ptr = bytes.baseAddress else {return}
			sizeWritten = fwrite(ptr, 1, count, file)
		}
	...
	func scanValue<T>(start: Int) -> T {
		subdata(in: start..<start + MemoryLayout<T>.size)
			.withUnsafeBytes{ (bytes: UnsafeRawBufferPointer) in
